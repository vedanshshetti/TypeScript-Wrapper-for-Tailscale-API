...
export default async function createPolicyFileSetter(apiKey: APIKey, tailnet: string, body: SetPolicyFileBody): Promise<GetPolicyFileReturnType> {
    const req = await authorisedFetch(`${TailscaleAPIBaseURL}/tailnet/${tailnet}/acl`, apiKey, {method: "POST", body: JSON.stringify(body)}, true);
    if (!req.ok) throw new Error(buildErrorMessage(`Operation policyFile.setPolicyFile with API Key ${hideApiKey(apiKey)} and

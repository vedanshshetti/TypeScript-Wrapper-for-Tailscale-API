...
export default async function createPreviewRuleMatches(apiKey: APIKey, tailnet: string, body: SetPolicyFileBody, queryString: `?type=${"user" | "ipport"}&previewFor=${IPAddressWithPort | `${string}@${string}`}`): Promise<PreviewRuleMatchesReturnType> {
    ...
    if (!req.ok) throw new Error(buildErrorMessage(`Operation policyFile.previewRuleMatches with API Key ${hideApiKey(apiKey)} and

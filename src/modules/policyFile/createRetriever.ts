...
export default async function createRetriever(apiKey: APIKey, tailnet: string): Promise<GetPolicyFileReturnType> {
    const req = await authorisedFetch(`${TailscaleAPIBaseURL}/tailnet/${tailnet}/acl`, apiKey);
    if (!req.ok) throw new Error(buildErrorMessage(`Operation policyFile.getPolicyFile with API Key ${hideApiKey(apiKey)} and

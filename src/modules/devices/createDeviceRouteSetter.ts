...
export default async function createDeviceRouteSetter(apiKey: APIKey, deviceID: string, body: SetDeviceRoutesBody): Promise<SetDeviceRoutesReturnType> {
    ...
    if (!req.ok) throw new Error(buildErrorMessage(`Operation devices.setDeviceRoutes with API Key ${hideApiKey(apiKey)} failed;

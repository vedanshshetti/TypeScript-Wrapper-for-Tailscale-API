...
import {
  APIKey,
  GetPolicyFileReturnType,
  IPAddressWithPort,
  PreviewRuleMatchesReturnType,
  SetPolicyFileBody,
...
const createPolicyFileInterface = (apiKey: APIKey, tailnet: string) => {
  return {
    ...
    getPolicyFile: (): Promise<GetPolicyFileReturnType> =>
      createRetriever(apiKey, tailnet),
    /**
    ...
    setPolicyFile: (
      ...
      createPolicyFileSetter(apiKey, tailnet, body),
    /**
     * Check
    ...
    previewRuleMatches: (
      body: SetPolicyFileBody,
      type: "user" | "ipport",
      previewFor: `${string}@${string}` | IPAddressWithPort
    ): Promise<PreviewRuleMatchesReturnType> =>
      createPreviewRuleMatches(
        apiKey,
        tailnet,
        body,
        `?type=${type}&previewFor=${previewFor}`
      ),
    /**
     * Run ACL Tests.
    ...
    validateAndTestPolicyFile: (
      ...
      createPolicyFileValAndTest(apiKey, tailnet, body)
  };
};
